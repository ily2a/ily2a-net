...
let ipCounter = 0
function makeReq(body: unknown, { ip, headers = {} }: ReqOpts = {}): Request {
  const raw = typeof body === 'string' ? body : JSON.stringify(body)
  const realIp = ip ?? `10.0.0.${++ipCounter}`
  return {
    headers: { get: (k: string) => (k === 'x-real-ip' ? realIp : headers[k] ?? null) },
    text: async () => raw,
...
describe('POST /api/contact', () => {
  ...
  it('returns 500 when the send exceeds the 8s timeout', async () => {
    ...
    try {
      ...
      sendMock.mockReturnValueOnce(new Promise<unknown>((resolve) => { release = resolve }))
