...
const GLASS_STYLE_MOBILE: CSSProperties  = { maxWidth: '440px', minWidth: 'auto',  flexShrink: 0, boxSizing: 'border-box' }
const GLASS_STYLE_DESKTOP: CSSProperties = { maxWidth: 'none',  minWidth: '472px', flexShrink: 0, boxSizing: 'border-box' }

const VALID_SECTIONS = new Set(['hero', 'work', 'capabilities', 'contact'])

const Navbar = memo(function Navbar({ isMobile = false }: { isMobile?: boolean }) {
  ...
  function navTo(sectionId: string) {
    if (!VALID_SECTIONS.has(sectionId)) return
    const focusSection = (el: HTMLElement | null) => {
      if (!el) return
