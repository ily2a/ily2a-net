...
// Below tab breakpoint (730px): image + below-image text layout.
// At tab breakpoint and above: full hover-overlay layout.
const ProjectCard = memo(function ProjectCard({ project }) {
  return (
    ...
    >
      {/* ── Image container — shared by both layouts ── */}
      <motion.div variants={imgVariants} className="relative aspect-[16/10] w-full rounded-xl overflow-hidden">
        ...
        {/* Hover image — desktop only; no hover on touch so no need to render on mobile */}
        ...
        {/* Mobile: tags inside image at bottom-left (hidden on desktop) */}
        <ul className="project-card-mobile__tags tab:hidden" role="list">
          {project.tags?.map((tag) => (
            <li key={tag} className="project-card__tag">{tag}</li>
          ))}
        </ul>

        {/* Desktop: hover overlay blur, scrim, and content (hidden on mobile) */}
        <motion.div className="project-card__blur hidden tab:block" aria-hidden="true" variants={overlayVariants}>
          {BLUR_LAYERS.map((layer) => (
            <div key={layer.blur} className="project-card__blur-layer" style={layer.style} />
          ))}
        </motion.div>
        <motion.div className="project-card__scrim hidden tab:block" variants={overlayVariants} />
        <motion.div className="project-card__content hidden tab:flex" variants={contentVariants}>
      ...
      {/* ── Mobile: text below image (hidden on desktop) ── */}
      <div className="project-card-mobile__content tab:hidden">
        <div className="project-card__text">
          <h3 className="project-card__title heading-2">{project.title}</h3>
          <p className="project-card__subtitle font-normal tracking-[0.07em] leading-[150%] text-[14px] md:text-[16px] xl:text-[20px] text-balance">{project.description}</p>
        </div>
