...
import { useState, useId, useRef, useEffect, type ChangeEventHandler, type ReactNode, type RefObject } from 'react'
...
function useDims(ref: RefObject<HTMLElement | null>): Dims {
...
function NotchedBorder({ floated, hasError, focused, labelWidth, width, height }: NotchedBorderProps) {
...
function FloatingLabel({ htmlFor, labelRef, floated, hasError, focused, children, restingTop = '50%', required }: FloatingLabelProps) {
  return (
    <m.label
      htmlFor={htmlFor}
