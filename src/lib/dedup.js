...
export function dedupKey(secret, email, message) {
...
// fails.
export function createDedupCache({ ttl = DEFAULT_DEDUP_TTL, now = Date.now } = {}) {
  const cache = new Map()
...
export const contactDedup = {
  key: (email, message) => dedupKey(DEDUP_SECRET, email, message),
