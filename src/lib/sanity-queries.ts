...
export const CASE_STUDY_BY_SLUG_QUERY = `*[_type == "caseStudy" && slug.current == $slug][0] {
  _id, _createdAt, _updatedAt, title, slug, client, role, timeline, platform, industry, tags, description, isPasswordProtected,
  coverImage { ..., "lqip": asset->metadata.lqip, "dimensions": asset->metadata.dimensions, "url": asset->url, alt },
  brief, problem, goals, uxStrategy,
  body[] {
    ...,
    _type == "image" => { ..., "lqip": asset->metadata.lqip, "dimensions": asset->metadata.dimensions }
  },
  figmaEmbed
}`
