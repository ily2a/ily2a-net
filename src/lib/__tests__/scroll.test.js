// Tests for the lib/scroll.js identity-guard semantics. We mock framer-motion
...
describe('scrollToY', () => {
  ...
  it('onComplete only nulls `current` when current still equals the issuing controller', () => {
    ...
    const second = makeControls()
    mockAnimate
      .mockImplementationOnce((_a, _b, opts) => { first.opts = opts; return first })
      .mockImplementationOnce((_a, _b, opts) => { second.opts = opts; return second })
...
describe('scrollToElement', () => {
  it('returns null when element is falsy', () => {
    ...
    expect(scrollMod.scrollToElement(undefined)).toBeNull()
  })
